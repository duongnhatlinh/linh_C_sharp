...
{
    public class PermissViewFileGroupController : Controller
    {
        const int idadmin = 24101;
        private readonly hethongquanlyluutrufileContext _context;
        public INotyfService _notyfService { get; }

        public PermissViewFileGroupController(hethongquanlyluutrufileContext context, INotyfService notyfService)
        {
            _context = context;
        ...
        public IActionResult SelectGroupUsers(int check)
        {
            ...
            switch (check)
            {
                case 1: // phòng nhân sự
                    lsuser = _context.Users.Where(x => x.DepartmentId == 1 && x.UserId != Convert.ToInt32(userID) && x.UserId != idadmin).ToList();
                    TempData["department"] = "Phòng Nhân Sự";
                    break;
                case 2: // phòng kế toán
                    lsuser = _context.Users.Where(x => x.DepartmentId == 2 && x.UserId != Convert.ToInt32(userID) && x.UserId != idadmin).ToList();
                    TempData["department"] = "Phòng Kế Toán";
                    break;
                case 3: // phòng kinh doanh
                    lsuser = _context.Users.Where(x => x.DepartmentId == 3 && x.UserId != Convert.ToInt32(userID) && x.UserId != idadmin).ToList();
                    TempData["department"] = "Phòng Kinh Doanh";
                    break;
                case 4: // phòng kỹ thuật
                    lsuser = _context.Users.Where(x => x.DepartmentId == 4 && x.UserId != Convert.ToInt32(userID) && x.UserId != idadmin).ToList();
                    TempData["department"] = "Phòng Kỹ Thuật";
                    break;
                case 5: // phòng sản xuất
                    lsuser = _context.Users.Where(x => x.DepartmentId == 5 && x.UserId != Convert.ToInt32(userID) && x.UserId != idadmin).ToList();
        ...
        public async Task<IActionResult> PermissGroupUsers(int? fileid)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> PermissGroupUsers(int fileid, SharedFile s)
        {
